...
| P2 | MEDIA-02 | in progress | Validation Architect | MEDIA-01; FIG-01 | deterministic accepted/revised/rejected dry run, retained decision reasons, exact input/output hashes and stale-asset tests; final-head CI pending | The media lifecycle is auditable without ordinary CI regeneration |
...
The deterministic dry run contains exactly one accepted, revised and rejected record. Revised and rejected candidates remain cached but unpublished, with explicit human-review reasons. The validator recomputes committed input/output hashes and byte counts and rejects external generation in ordinary CI. Passing CI can establish lifecycle-contract integration only; it does not establish generative-media usefulness, technical correctness or educational benefit.
