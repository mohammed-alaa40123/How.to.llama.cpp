...
- [ ] Audit pinned RPC teardown: client/session/socket lifetime, outstanding request completion, remote buffer release, scheduler behavior, error paths, and backend-before-scheduler classification.
- [ ] Continue concrete teardown audits for CANN and OpenCL, including optional CPU extra-buffer implementations.
...
- [ ] Add asynchronous-destruction regression tests for CUDA-family, Metal, Vulkan, and SYCL backends.
...
- [ ] Verify the public Pages site returns HTTP 200 and renders `architecture/sycl-backend-teardown/` with expected How.to.llama.cpp content.
